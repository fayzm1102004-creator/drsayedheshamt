...
export default function ShamelaWidget({ onExtract }) {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (panelRef.current && !panelRef.current.contains(event.target)) {
        // Keep it manual close for better UX so they don't accidentally lose results
      }
    };
  ...
  return (
    <div className="fixed bottom-4 sm:bottom-6 left-4 sm:left-6 z-[100]" dir="rtl" ref={panelRef}>
      ...
      {isOpen && (
        <>
          ...
          ">
            ...
            {/* Header */}
            <div className="bg-gradient-to-r from-[#064e3b] to-emerald-800 p-3 sm:p-4 flex items-center justify-between shadow-md relative overflow-hidden safe-area-top">
              ...
              <div className="flex items-center gap-2 sm:gap-3 relative z-10">
                <div className="bg-white/20 p-1.5 sm:p-2 rounded-lg backdrop-blur-sm">
                  <Library className="w-4 h-4 sm:w-5 sm:h-5 text-amber-300" />
                </div>
                <div>
                  <h3 className="text-white font-bold font-['Aref_Ruqaa'] text-base sm:text-lg leading-tight">المكتبة الشاملة</h3>
                  <p className="text-emerald-100/80 text-[10px] sm:text-xs">بحث واستخراج الشواهد</p>
                </div>
              </div>
              <button 
                onClick={() => setIsOpen(false)}
                className="relative z-10 text-white/70 hover:text-white hover:bg-white/20 p-1.5 sm:p-2 rounded-full transition-colors"
              >
            ...
            {/* Search Area */}
            <div className="p-3 sm:p-4 bg-slate-50 dark:bg-slate-950 border-b border-slate-100 dark:border-slate-800">
            ...
            {/* Results Area */}
            <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3 sm:space-y-4 bg-slate-50 dark:bg-slate-950/50 scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-700">
              ...
              {/* Error State */}
              {error && !isSearching && (
                <div className="bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-900 rounded-xl p-3 sm:p-4 flex gap-3 text-rose-700 dark:text-rose-400">
              ...
              {!isSearching && !error && hasSearched && results.length === 0 && (
                <div className="flex flex-col items-center justify-center py-10 text-center text-slate-500">
                  ...
                  <p className="font-bold text-sm">لم يتم العثور على نتائج</p>
                    ...
                    <div key={idx} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow group">
                      <div className="p-3 bg-[#064e3b]/5 dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700/50 flex justify-between items-start gap-2">
                        <div className="min-w-0">
                          <a href={result.sourceUrl} target="_blank" rel="noopener noreferrer" className="font-bold text-[#064e3b] dark:text-amber-400 text-xs sm:text-sm hover:underline line-clamp-1 block">
                            {result.bookTitle}
                          </a>
                          <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mt-1 truncate">{result.referenceInfo}</p>
                        </div>
                        <Book className="w-4 h-4 text-[#d4af37] shrink-0 mt-0.5" />
                      </div>
                      <div className="p-3 relative group">
                        <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 leading-relaxed text-justify line-clamp-4">
                          {result.quoteText}
                        </p>
      ...
      <button
        ...
        className={`w-12 h-12 sm:w-14 sm:h-14 rounded-full shadow-[0_10px_25px_rgba(6,78,59,0.3)] dark:shadow-[0_10px_25px_rgba(245,158,11,0.2)] flex items-center justify-center transition-all duration-300 hover:scale-110 active:scale-95 z-[120] relative ${
          isOpen 
      ...
      >
        ...
        {isOpen ? <ChevronDown className="w-5 h-5 sm:w-6 sm:h-6" /> : <Library className="w-5 h-5 sm:w-6 sm:h-6" />}
      </button>
